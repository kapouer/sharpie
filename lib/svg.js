		...
		if (begun !== false) {
			...
			if (stop >= 0) {
				...
				if (params.rs) {
					...
					if (resize && (params.rs.max && w > resize.width || params.rs.min && w < resize.width || params.rs.z)
					&& atts.viewBox) {
						atts.width = resize.width;
					} else {
						delete atts.width;
					}
					if (resize && (params.rs.max && h > resize.height || params.rs.min && h < resize.height || params.rs.z)
					&& atts.viewBox) {
						atts.height = resize.height;
					} else {
						delete atts.height;
					}
				}
