...
function doPipe(pipeline, req, res, next, request, response, resize, extract, params) {
	...
	var dstFormat = params.format;
	if (!dstFormat) {
		if (["png", "jpeg", "tiff"].includes(srcFormat)) {
			res.setHeader('Vary', 'Accept');
			if (req.get('Accept') && req.accepts(['image/png', 'image/jpeg', 'image/tiff', 'image/webp']) == "image/webp") {
				dstFormat = 'webp';
				if (srcFormat == "png") {
	...
	if (params.q) {
		...
		if (isNaN(formatOptions.quality)) {
			return res.type('text').status(400).send(`Bad q parameter`);
		}
	}
