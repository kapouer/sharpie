	...
	return function(req, res, next) {
		...
		.on('response', function(response) {
			...
			if (srcFormat in formats) {
				...
				if (srcFormat == "svg") {
					if (params.style) {
						request.pipe(svgTransform(params.style)).pipe(res);
					} else {
						request.pipe(res);
					}
				} else {
...
function svgTransform(style) {
	...
	return new stream.Transform({transform: function(chunk, encoding, cb) {
		...
		if (begun !== false) {
			...
			if (stop >= 0) {
				ended = true;
				// found end of svg tag
				chunk = Buffer.concat([chunk.slice(0, stop + 1), Buffer.from(svgStyle(style)), chunk.slice(stop + 1)]);
			}
		}
		cb(null, chunk);
	}});
