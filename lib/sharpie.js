...
const acceptsFormats = Object.keys(formats.dst).map((fmt) => `image/${fmt}`);
...
function doPipe(pipeline, req, res, next, request, response, resize, extract, params) {
	...
	if (!dstFormat) {
		...
		const accepts = req.accepts(acceptsFormats);
		if (srcFormat == "svg") {
			// no change
	...
	pipeline.metadata().then((meta) => {
		...
		if (!formatOptions.quality) formatOptions.quality = 80;
