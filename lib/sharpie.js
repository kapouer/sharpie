	...
	return new stream.Transform({transform: function(chunk, encoding, cb) {
		...
		if (begun !== false) {
			...
			if (stop >= 0) {
				...
				tagChunks.push(chunk.slice(start, stop));
				var tag = Buffer.concat(tagChunks);
				tagChunks = null;
				if (tag.indexOf('viewBox') >= 0 && tag.indexOf('preserveAspectRatio') < 0) {
					bufs.push(Buffer.from(` preserveAspectRatio="${params.ratio || 'xMinYMin'}"`));
				}
