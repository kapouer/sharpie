...
function mw(req, res, next) {
	...
	const subReq = agent.request(urlObj, opts.https || {}, subRes => {
		if (subRes.statusCode != 200) {
			...
			doPipe(opts, subRes, req, res, next, resize, extract, params);
		}
	});
	subReq.on('error', err => {
...
function doPipe(opts, response, req, res, next, resize, extract, params) {
	...
	if (srcFormat == "svg" && dstFormat == "svg") {
		response.pipe(svg.transform(params, resize, extract)).pipe(res);
		return;
	}
	if (dstFormat == "ico") {
