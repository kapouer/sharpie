			...
			if (srcFormat in formats) {
				...
				try {
					pipeline.metadata().then(function(meta) {
						if (extract) {
							var exOpts = {
								left: Math.round((extract.x - extract.w / 2) * meta.width / 100),
								top: Math.round((extract.y - extract.h / 2) * meta.height / 100),
								width: Math.round(extract.w * meta.width / 100),
								height: Math.round(extract.h * meta.height / 100)
							};
