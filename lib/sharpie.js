...
module.exports = function(defaults) {
	...
	return function(req, res, next) {
		...
		var pipeline = sharp().flatten().withoutEnlargement()
		.resize(resize.w, resize.h).crop('centre');
		if (resize.max) pipeline = pipeline.max();
		if (resize.min) pipeline = pipeline.min();
		pipeline = pipeline.background('white').jpeg();
